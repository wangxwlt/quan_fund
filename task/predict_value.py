...
my_util.fund_history_net_asset_em('000016')
...
def _make_sample(label_data):
    ...
    date = label_data["日期"]
    label_sr = pd.Series({'label': label_data['7天后累计净值变化率']})
...
all_funds_value = pd.read_csv(config.FUND_VALUE_FILE, dtype=config.COL_TYPE)
all_funds_value = all_funds_value.set_index([config.INDEX_COL, '日期'])
all_funds_value.index.names = [config.INDEX_COL, '日期']
all_funds_label = pd.read_csv(config.FUND_LABEL_FILE, dtype=config.COL_TYPE, nrows=100)
